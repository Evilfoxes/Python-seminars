...
def check_rhythm(stroka):
    phrases = stroka.lower().split(' ')
    if len(phrases) < 2:
        return 'Количество фраз должно быть больше одной!'

    vowel_letters = 'ауоиэыяюеё'
    vowel_nums = [[phrase.count(vl) for vl in vowel_letters] for phrase in phrases]

    return ('Пам парам', 'Парам пам-пам')[all([sum(vowel_nums[0]) == sum(nums) for nums in vowel_nums[1:]])]
